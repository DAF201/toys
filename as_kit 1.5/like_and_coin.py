...
def like_and_coins(video_data,login_data):
    ...
    reply=requests.get(check_like_url,params=params_check,cookies=cookie).text
    reply=json.loads(reply)
    if reply['data']['multiply']==0:
        reply=requests.post(like_url,params=params_like_and_coins,cookies=cookie).text
        reply=json.loads(reply)
